...
<template>
    ...
    <AuthenticatedLayout>
        <!-- Top bar -->
        <div
            class="flex flex-col gap-3 border-b border-rule px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-9 sm:py-6"
        >
            <div>
                <h1
                    class="font-display text-2xl font-medium tracking-tight text-ink"
                >
                    Overview
                </h1>
        ...
        <div class="space-y-8 px-5 py-6 sm:px-9 sm:py-8">
            <!-- Pending reviews -->
            <div
            ...
            >
                ...
                <span
                    ...
                    awaiting open-text review.</span
                >
            </div>
            ...
            <section class="grid grid-cols-2 gap-3 lg:grid-cols-4">
                ...
                >
                    <p
                        class="mb-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-ink-mute"
                    >
                        {{ item.label }}
                    </p>
                    <p
            ...
            <section>
                <header class="mb-3.5 flex items-center justify-between">
                    <h2
                        class="text-[13px] font-bold uppercase tracking-[0.06em] text-ink"
                    >
                        Recent Exams
                    </h2>
                    <Link
                        :href="route('lecturer.exams.index')"
                        class="text-[11px] font-semibold tracking-wide text-ink-mute transition hover:text-ink"
                    >
                        View all →
                    </Link>
                </header>
                <div class="border border-rule bg-white">
                    ...
                    >
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="truncate text-[11px] text-ink-mute">
                                {{ ex.subject }}
                            </p>
                        </div>
                        <span
                            ...
                            :class="[
                                badgeFor(ex.status).bg,
                                badgeFor(ex.status).text,
                            ]"
                        >
                            <span
                                class="h-1.5 w-1.5 rounded-full"
                                :class="badgeFor(ex.status).dot"
                            />
                            {{ ex.status }}
                        </span>
                        <span
                            class="hidden w-16 flex-shrink-0 text-right text-xs text-ink-mute md:block"
                            >{{ ex.time_limit }}min</span
                        >
                        <span
            ...
            <section>
                ...
                >
                    ...
                    >
                        ...
                        <p class="mt-1 text-[11px] text-ink-mute">
                            {{ cl.students }} student{{
                                cl.students === 1 ? "" : "s"
                            }}
                        </p>
                        <div class="mt-3 flex flex-wrap gap-1">
                            ...
                            >
                            <span
                                v-if="!cl.subjects?.length"
                                class="text-[10px] italic text-ink-mute"
                                >No subjects</span
                            >
                        </div>
                    </div>
